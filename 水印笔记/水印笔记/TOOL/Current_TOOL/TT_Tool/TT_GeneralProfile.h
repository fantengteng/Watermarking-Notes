...
security_H = [UIScreen mainScreen].bounds.size.height - securityBottom_H - securitytop_Y ;\
...
/**  收通知  */
#define ResiveMessage(name,DoSoming) [[NSNotificationCenter defaultCenter] addObserverForName:name object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) { TT_Log(@"%@",note); DoSoming }];
/**  移除通知  */
...
#define DefaultBackIMG @"fanhui"

#endif /* TT_GeneralProfile_h */
