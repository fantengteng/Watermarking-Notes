...
- (void)configFailTankuang:(NSString *)mark {
    [[FTT_HudTool share_FTT_HudTool]CreateHUD:LOCALIZATION(@"加载失败,请重试!!!")  AndView:self.view AndMode:MBProgressHUDModeText AndImage:@"NONONO" AndAfterDelay:1 AndBack:nil];
}
...
#pragma mark 私有方法
- (void)CreateBack {
    UIButton *btn = [TT_ControlTool FTT_ControlToolUIButtonFrame:CGRectMake(0, 0, 40, 40) taeget:self
                                                             sel:@selector(BackBarButtonPressed:)
                                                             tag:0
                                                        AntTitle:nil
                                                       titleFont:0
                                                      titleColor:nil
                                                        andImage:DefaultBackIMG
                                                    AndBackColor:nil
    ...
    UIBarButtonItem *imageItem = [[UIBarButtonItem alloc]initWithCustomView:btn];
